...
import { ReplicateService } from '@/lib/llm/replicate'; // Using path alias for Replicate
...
const replicateService = new ReplicateService(); // Instantiate ReplicateService
...
if (!process.env.REPLICATE_API_TOKEN) {
  console.warn("REPLICATE_API_TOKEN is not set. Replicate models will not be available.");
}
...
console.log("API Keys availability check:", {
  ...
  REPLICATE_API_TOKEN_SET: !!process.env.REPLICATE_API_TOKEN, // Added Replicate API token check
...
export async function POST(request: Request) {
  try {
    ...
    // --- Handle Replicate/Bagel Model --- This condition must come BEFORE the generic getServiceForModel call
    else if (modelName === 'bytedance/bagel' || modelName === 'black-forest-labs/flux-kontext-pro') {
      if (!process.env.REPLICATE_API_TOKEN) {
        return NextResponse.json({ error: `REPLICATE_API_TOKEN is not set. Replicate (${modelName}) model is not available.` }, { status: 503 });
      }
      try {
        // The ReplicateService is now specialized for Bagel, so modelName can be passed for consistency if needed by the interface
        // but the service itself uses the hardcoded BAGEL_MODEL_ID.
        const streamIterable = await replicateService.generateResponse(processedMessage ?? '', serviceHistory, fileData, fileUri, modelName);
        ...
        const readableStream = new ReadableStream({
          async start(controller) {
            ...
            try {
              ...
              console.error(`Error reading from Replicate stream (${modelName}):`, error);
              const errorPayload = { text: `[Error: ${error instanceof Error ? error.message : `Unknown streaming error with ${modelName}`}]` };
              controller.enqueue(encoder.encode(JSON.stringify(errorPayload) + '\n'));
              controller.close();
            }
          },
          cancel() {
            console.log(`Replicate stream (${modelName}) cancelled by client.`);
          }
        });
        ...
        console.error(`API Route Error (Replicate Handler - ${modelName}):`, error);
        const errorMessage = error instanceof Error ? error.message : `Error processing Replicate request for ${modelName}`;
        return NextResponse.json({ error: errorMessage }, { status: 500 });
      }
    }
    // --- End Handle Replicate/Bagel Model ---

    // Select the appropriate service based on model name for Gemini/Perplexity (Default cases)
